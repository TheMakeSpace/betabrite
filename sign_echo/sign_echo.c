...
void print_usage(char *cmd_name)
{
    ...
    for (i = 0; i < NUM_SPECIAL_MODES; i++) {
        fprintf(stdout, "        %d = %s\n", i, special_modes[i].name);
    }
};
        ...
        switch (ret) {
            case 0:
                if (strcmp(long_options[option_index].name,"port") == 0) {
                    ...
                    msg_special_mode = atoi(optarg);
                    if (msg_special_mode < 0 || msg_special_mode >= NUM_SPECIAL_MODES) {
            ...
            case 'p':
                msg_pos = atoi(optarg);
                if (msg_pos < 0 || msg_pos >= NUM_POSITIONS) {
                    fprintf(stderr, "error: invalid message position\n");
                    return -1;
                }
                break;
            case 's':
                msg_speed = atoi(optarg);
                if (msg_speed < 0 || msg_speed >= NUM_SPEEDS) {
                    fprintf(stderr, "error: invalid message speed\n");
                    return -1;
                }
                break;
            case 'c':
                msg_color = atoi(optarg);
                if (msg_color < 0 || msg_color >= NUM_COLORS) {
                    fprintf(stderr, "error: invalid color\n");
                    return -1;
                }
                break;
            case 'm':
                msg_mode = atoi(optarg);
                if (msg_mode < 0 || msg_mode >= NUM_MODES) {
